for j = 1:1:length(Celloutputnondiv)
    xstart = -6;
    xstop = 6;
    Celloutputnondiv(j).meas(:,42) = NaN;
    Celloutputnondiv(j).meas(:,43) = NaN;
    Celloutputnondiv(j).meas(:,44) = NaN;
    Celloutputnondiv(j).meas(:,45) = NaN;
    Celloutputnondiv(j).meas(:,46) = NaN;
    Celloutputnondiv(j).meas(:,47) = NaN;
    foa = Celloutputnondiv(j).scoring(1,2);
    ioa = find ((Celloutputnondiv(j).meas(:,1))== foa);
    if ~isnan(foa)
        for h = 1:1:length(Celloutputnondiv(j).Area_vert_Ch1(:,1))
            
            if ~isnan(Celloutputnondiv(j).Area_vert_Ch1(h,1))
                ...
                yyaxis right
                Y1 = Celloutputnondiv(j).RawIntDen_vert_Ch1(h,:)./Celloutputnondiv(j).Area_vert_Ch1(h,:);
                plot(X1,Y1,'Marker','o','Color',[1 0 0]);
                xlim([-6 6])
                
                yyaxis left
                Y1 = Celloutputnondiv(j).RawIntDen_vert_Ch2(h,:)./Celloutputnondiv(j).Area_vert_Ch2(h,:);
                ...
                if ~isnan(Fs)
                    ...
                    Celloutputnondiv(j).meas(h,42) =  Celloutputnondiv(j).RawIntDen_vert_Ch2(h,wii)/Celloutputnondiv(j).Area_vert_Ch2(h,wii);
                    Celloutputnondiv(j).meas(h,43) =  Fs;
                    Celloutputnondiv(j).meas(h,46) = Celloutputnondiv(j).RawIntDen_vert_Ch1(h,wii)/Celloutputnondiv(j).Area_vert_Ch1(h,wii);
                end
                if ~isnan(Ff)
                    
                    waa = find ((IndexTranslation(:,2)) == Ff);
                    wii = IndexTranslation(waa,1);
                    Celloutputnondiv(j).meas(h,44) =  Celloutputnondiv(j).RawIntDen_vert_Ch2(h,wii)/Celloutputnondiv(j).Area_vert_Ch2(h,wii);
                    Celloutputnondiv(j).meas(h,45) =  Ff;
                    Celloutputnondiv(j).meas(h,47) = Celloutputnondiv(j).RawIntDen_vert_Ch1(h,wii)/Celloutputnondiv(j).Area_vert_Ch1(h,wii);
                end
                close(figure1);
            end
        end
