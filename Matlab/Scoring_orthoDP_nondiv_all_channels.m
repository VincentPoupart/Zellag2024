for j = 1:1:length(Celloutputnondiv)
    ...
    Celloutputnondiv(j).meas(:,52) = NaN;
    Celloutputnondiv(j).meas(:,53) = NaN;
    
    foa = Celloutputnondiv(j).scoring(1,2);
    ioa = find ((Celloutputnondiv(j).meas(:,1))== foa);
    if ~isnan(foa)
        for h = 1:1:length(Celloutputnondiv(j).Area_orthoDP_Ch1(:,1))
            
            if ~isnan(Celloutputnondiv(j).Area_orthoDP_Ch1(h,1))
                ...
                yyaxis right
                Y1 = Celloutputnondiv(j).RawIntDen_orthoDP_Ch1(h,:)./Celloutputnondiv(j).Area_orthoDP_Ch1(h,:);
                plot(X1,Y1,'Marker','o','Color',[1 0 0]);
                xlim([-6 6])
                
                yyaxis left
                Y1 = Celloutputnondiv(j).RawIntDen_orthoDP_Ch2(h,:)./Celloutputnondiv(j).Area_orthoDP_Ch2(h,:);
                ...
                if ~isnan(Fs)
                    ...
                    Celloutputnondiv(j).meas(h,48) =  Celloutputnondiv(j).RawIntDen_orthoDP_Ch2(h,wii)/Celloutputnondiv(j).Area_orthoDP_Ch2(h,wii);
                    Celloutputnondiv(j).meas(h,49) =  Fs;
                    Celloutputnondiv(j).meas(h,52) =  Celloutputnondiv(j).RawIntDen_orthoDP_Ch1(h,wii)/Celloutputnondiv(j).Area_orthoDP_Ch1(h,wii);
                end
                if ~isnan(Ff)
                    
                    waa = find ((IndexTranslation(:,2)) == Ff);
                    wii = IndexTranslation(waa,1);
                    Celloutputnondiv(j).meas(h,50) =  Celloutputnondiv(j).RawIntDen_orthoDP_Ch2(h,wii)/Celloutputnondiv(j).Area_orthoDP_Ch2(h,wii);
                    Celloutputnondiv(j).meas(h,51) =  Ff;
                    Celloutputnondiv(j).meas(h,53) =  Celloutputnondiv(j).RawIntDen_orthoDP_Ch1(h,wii)/Celloutputnondiv(j).Area_orthoDP_Ch1(h,wii);
                end
                close(figure1);
            end
        end
