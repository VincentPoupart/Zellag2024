folder = uigetdir;
mkdir(folder, 'OrthoVectorLineScan');
...
for j = 1:1:length(Celloutput)
    ...
    Celloutput(j).meas(:,27) = NaN;
    Celloutput(j).meas(:,28) = NaN;
    Celloutput(j).meas(:,36) = NaN;
    Celloutput(j).meas(:,37) = NaN;
            ...
            if h == ioa
                ...
                if ~isnan(Celloutput(j).Area_ortho_Ch1(h,1))
                    ...
                    if ~isnan(Fs)
                        ...
                        Celloutput(j).meas(h,26) =  Fs;
                        Celloutput(j).meas(h,36) = Celloutput(j).RawIntDen_ortho_Ch1(h,wii)/Celloutput(j).Area_ortho_Ch1(h,wii);
                    end
                    if ~isnan(Fe)
                        
                        waa = find ((IndexTranslation(:,2)) == Fe);
                        wii = IndexTranslation(waa,1);
                        Celloutput(j).meas(h,27) =  Celloutput(j).RawIntDen_ortho_Ch2(h,wii)/Celloutput(j).Area_ortho_Ch2(h,wii);
                        Celloutput(j).meas(h,28) =  Fe;
                        Celloutput(j).meas(h,37) = Celloutput(j).RawIntDen_ortho_Ch1(h,wii)/Celloutput(j).Area_ortho_Ch1(h,wii);
                    end
                    close(figure1);
                end
            end
