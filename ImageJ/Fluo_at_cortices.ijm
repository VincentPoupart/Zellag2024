...
function GetFiles(fazer) {//////// get txt files and send to sphere function
	lista = getFileList(fazer);
	Array.sort(lista); 
...
function GetFiles2(BINARYdir) {//////// get tiff binary files and send to gonadeSeg function
...
function MeasureFluoCh1(path) {//////////measure background in a small sphere around the SM
run("Close All");
run("Collect Garbage");
if (isOpen("Log")) {
         selectWindow("Log");
...
function MeasureFluoCh2(path) {//////////measure background in a small sphere around the SM
run("Close All");
run("Collect Garbage");
if (isOpen("Log")) {
         selectWindow("Log");
...
close();
                    ...
                    selectImage("C2-"+F+".tif");
                    ResultsFluo();////measure the FI and arrange results
                    saveAs("Results", ""+Spheres_at_Patchdirii+""+zipfiles[l]+"_channel_2.txt");
                    wait(500);
                    if (isOpen("Results")) { 
                    selectWindow("Results"); 
                    run("Close"); 
