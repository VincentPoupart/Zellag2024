...
CameraBackgrounddir = ""+root+"\\"+bo+"\\"+boby6+"\\";

File.makeDirectory(CameraBackgrounddir);
...
function GetFiles(fazer) { 
	...
	for (o=0; o<MoviesList.length; o++){
		path = ""+MoviesList[o]+"";
		CameraBackground(path);	
		}
	}


function CameraBackground(PathToMovie) {
	run("Close All");
    run("Collect Garbage");
	...
	for (m=0; m<sizeT; m++){
		...
		for (m=0; m<Area.length; m++){
		 ...
		 saveAs("Results", ""+CameraBackgrounddir+""+F+"_Camerabackground.csv");
		 run("Clear Results");
	}
