...
BleachingCurvedir = ""+root+"\\"+bo+"\\"+boby6+"\\";

File.makeDirectory(BleachingCurvedir);
...
function GetFiles(fazer) { 
	...
	for (o=0; o<MoviesList.length; o++){
		path = ""+MoviesList[o]+"";
		MeasureBleaching(path);	
		}
	}


function MeasureBleaching(PathToMovie) {
	run("Close All");
    run("Collect Garbage");
	...
	for (m=0; m<sizeT; m++){
		...
		for (m=0; m<Area.length; m++){
		 ...
		 saveAs("Results", ""+BleachingCurvedir+""+F+"_bleaching.csv");
		 run("Clear Results");
	}
