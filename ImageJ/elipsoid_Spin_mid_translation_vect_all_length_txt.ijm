...
close();
                    ...
                    saveAs("Results", ""+Spheres_at_Patchdirii+""+zipfiles[l]+".txt");
                    wait(500);
                    if (isOpen("Results")) { 
...
function sphere(path) {//////draw the spheres on the size according to matlab, please adjust the radius because the segmentation of the rachis is not perfect
  ...
  for (j=1; j<(sizeT+1); j++) {
   ...
   run("3D Draw Shape", "size="+width+","+height+","+slices+" res_xy="+sizeX+" res_z="+sizeZ+" unit=microns centre="+xcoord[j-min]+","+ycoord[j-min]+","+zcoord[j-min]+" radius="+radius[j-min]+","+radius2[j-min]+","+radius2[j-min]+" vector1="+V1x[j-min]+","+V1y[j-min]+","+V1z[j-min]+" vector2="+V2x[j-min]+","+V2y[j-min]+","+V2z[j-min]+" value=255 display=[New stack]");
   selectImage("Shape3D");
