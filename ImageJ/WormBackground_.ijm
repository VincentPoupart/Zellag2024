...
WormBackgrounddir = ""+root+"\\"+bo+"\\"+boby6+"\\";

File.makeDirectory(WormBackgrounddir);
...
function WormBackground(PathToMovie) {
	run("Close All");
    run("Collect Garbage");
	...
	for (m=0; m<sizeT; m++){
		...
		for (m=0; m<Area.length; m++){
		 ...
		 saveAs("Results", ""+WormBackgrounddir+""+F+"_Wormbackground.csv");
		 run("Clear Results");
	}
